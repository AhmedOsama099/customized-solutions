...
/** Arrow that flips horizontally under RTL so it points with the reading flow. */
function FlowArrow() {
  return <ArrowRight size={17} strokeWidth={2.4} className="rtl:-scale-x-100" aria-hidden="true" />;
}
...
function Header({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  const nav = [
    { href: "#what", label: dict.nav.whatWeDo },
    ...
    <header className="sticky top-0 z-50 border-b border-border-soft bg-[rgba(246,247,251,0.78)] backdrop-blur-[14px]">
      <div className="mx-auto flex max-w-[1120px] items-center justify-between gap-6 px-7 py-[14px]">
        <Link href="#top" className="flex items-center gap-[11px] text-ink">
          <CustomizedMark size={34} />
          <span className="font-display text-[19px] font-bold tracking-[-0.02em]">
            {SITE.name}
          </span>
        </Link>
        <nav className="flex items-center gap-1.5 text-sm" aria-label={dict.nav.work}>
          {nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
          ...
          <LanguageSwitcher
            ...
            aria={dict.langSwitch.aria}
            className="inline-flex items-center gap-1.5 rounded-full border border-border-cta px-[13px] py-[7px] text-[13px] font-medium text-muted transition-colors hover:text-ink"
          />
          <a
            href="#contact"
            className="ms-1.5 rounded-full bg-accent px-[18px] py-[9px] font-semibold text-white shadow-[0_8px_18px_-10px_rgba(91,108,255,0.8)] transition-transform duration-150 hover:-translate-y-0.5"
          >
...
function Hero({ dict }: { dict: Dictionary }) {
  const [before, after] = dict.hero.body.split("{mihna}");
  return (
    <section
    ...
    >
      ...
      <div className="relative mx-auto max-w-[920px] px-7 pb-[100px] pt-[90px] text-center">
        ...
        <div
          dir="ltr"
          className="mt-10 text-[12.5px] tracking-[0.05em] text-muted-3"
...
function CapabilityStrip({ dict }: { dict: Dictionary }) {
  return (
        ...
        {dict.capabilities.map((cap, i) => (
          <div
            ...
            className={cn(
              ...
              i % 2 === 0 && "border-e border-border-line",
              i < 3 && "md:border-e md:border-border-line",
            )}
          >
...
function WhatWeDo({ dict }: { dict: Dictionary }) {
  return (
    <section id="what" className="mx-auto max-w-[1120px] px-7 py-24">
      <SectionLabel>{dict.what.label}</SectionLabel>
      ...
      <div className="mt-11 grid gap-[18px] md:grid-cols-2">
        {dict.what.items.map((s) => (
          ...
          >
            ...
            <p className="mt-[9px] text-[15px] leading-[1.62] text-muted">{s.body}</p>
          </div>
        ))}
      </div>
    </section>
...
function Work({ dict }: { dict: Dictionary }) {
  return (
    <section id="work" className="border-y border-border-soft bg-band">
        ...
        <div className="mt-11 grid grid-cols-1 overflow-hidden rounded-[24px] border border-[rgba(13,148,136,0.28)] bg-white shadow-[0_20px_50px_-30px_rgba(13,148,136,0.45)] md:grid-cols-[1.05fr_0.95fr]">
          <div className="p-10">
            <div className="flex items-center gap-[13px]">
              ...
              <div>
                <p className="font-display text-[24px] font-bold tracking-[-0.02em]">
                  mihna <span className="font-arabic font-bold text-teal">· مهنة</span>
                </p>
                <p className="mt-0.5 font-mono text-[11px] tracking-[0.05em] text-teal">
                  {dict.work.mihna.live}
                </p>
              </div>
            </div>
            <p className="mt-[22px] text-[16px] leading-[1.65] text-[#3f4654]">
              {dict.work.mihna.body}
            </p>
            <div className="mt-[22px] flex flex-wrap gap-2">
              {dict.work.mihna.tags.map((tag) => (
                <span
                  key={tag}
            ...
            <a
              ...
              className="mt-[26px] inline-flex items-center gap-2 text-[14.5px] font-semibold text-teal transition-transform duration-150 hover:translate-x-0.5"
            >
              {dict.work.mihna.visit} <FlowArrow />
            </a>
          </div>

          <div className="relative flex min-h-[280px] items-center justify-center border-t border-[rgba(13,148,136,0.16)] bg-[#f0fdf9] p-[30px] md:border-s md:border-t-0">
            <div
              ...
              style={{
                ...
                WebkitMaskImage: "radial-gradient(62% 62% at 50% 50%,#000,transparent)",
              }}
            />
...
function Approach({ dict }: { dict: Dictionary }) {
  return (
    <section id="approach" className="mx-auto max-w-[1120px] px-7 py-24">
      <SectionLabel>{dict.approach.label}</SectionLabel>
      ...
      <div className="mt-11 grid gap-[18px] md:grid-cols-3">
        {dict.approach.steps.map((step) => (
          ...
          >
            ...
            <h3 className="mt-3 font-display text-[20px] font-semibold">{step.title}</h3>
            <p className="mt-[9px] text-[14.5px] leading-[1.6] text-muted">{step.body}</p>
          </div>
        ))}
      </div>
    </section>
      ...
      >
        ...
        <div className="relative">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-[13px]">
            <a href={mailtoHref} className={primaryBtn}>
              {dict.cta.contact} <FlowArrow />
            </a>
            <a
              href={whatsappHref}
              target="_blank"
      ...
      <div className="mx-auto max-w-[1120px] px-7 py-11">
        <div className="flex flex-wrap items-center justify-between gap-8">
          <div className="flex items-center gap-[11px]">
            <CustomizedMark size={30} />
            <span className="font-display text-[17px] font-bold">{SITE.name}</span>
          </div>
          ...
          <div className="flex items-start gap-9">
            ...
            >
              ...
              <span className="text-[13px] font-medium">{dict.footer.portfolio}</span>
            </a>
          </div>
        </div>

        <p className="mt-9 border-t border-border-soft pt-7 text-center text-[13.5px] text-muted-3">
          {dict.footer.rights}
        </p>
      </div>
    </footer>
